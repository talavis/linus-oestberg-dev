...
In the teams case some data failed to sync, and thus the `postStart` and `preStop` commands never terminated, blocking the pod start and termination. After the data was synced correctly, everything worked perfectly.

As the hooks prevented the pod from starting and terminating, the manual container termination required a user with admin rights that could access the cluster nodes. To avoid this issue, avoid handling the container lifecycle hooks or, if they truly are needed, make sure to not prevent e.g. pod creation indefinitely and to make sure that the pod behaviour is well documented.
